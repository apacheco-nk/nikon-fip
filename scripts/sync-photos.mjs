...
for (const day of days) {
  ...
  const files = (await fs.readdir(source)).filter((file) =>
  ...
  for (const file of files) {
    ...
    const exif = await exifr
      .parse(input, { pick: ['DateTimeOriginal', 'CreateDate'] })
      .catch(() => null);
    const date = exif?.DateTimeOriginal || exif?.CreateDate;
    const capturedAt = date instanceof Date ? date.toISOString() : undefined;
    const fullName = `${stem}.jpg`;
    const thumbName = `${stem}.webp`;
    const optimizedPath = path.join(optimizedDir, fullName);
    const thumbPath = path.join(thumbDir, thumbName);
    const inputStat = await fs.stat(input);
    const current = await Promise.all([
      fs.stat(optimizedPath).catch(() => null),
      fs.stat(thumbPath).catch(() => null),
    ]);
    if (
      !current[0] ||
      !current[1] ||
      current[0].mtimeMs < inputStat.mtimeMs ||
      current[1].mtimeMs < inputStat.mtimeMs
    ) {
      await sharp(input)
        .rotate()
        .resize({
    ...
    photos.push({
      original: `/gallery/${day}/full/${encodeURIComponent(file)}`,
      optimized: `/gallery/${day}/optimized/${fullName}`,
      thumb: `/gallery/${day}/thumbs/${thumbName}`,
      filename: file,
      capturedAt,
      sort: capturedAt || file,
    });
  }
  photos.sort((a, b) => a.sort.localeCompare(b.sort));
  result[day] = photos.map(({ sort, ...photo }) => photo);
}
...
console.log(
  'Galería actualizada: ' +
    days.map((day) => day + ': ' + result[day].length).join(', '),
);
